...
#     Collects flattened gradients from the shared encoder parameters.
#     Returns {param_name: grad_flat} for params that have a gradient.
#     Clones explicitly so the tensors are independent of the model's grad buffers.
#     """
#     grads = {}
#     for name, param in model.encoder.named_parameters():
#         if param.grad is not None:
...
#     Zeros gradients first, runs backward, then snapshots and returns the shared
#     encoder gradients for this task.

#     The returned tensors are cloned CPU floats — safe to store while other
#     tasks' backward passes overwrite model.grad buffers.
#     """
#     model.zero_grad()
#     input_ids      = batch["input_ids"].to(device)
#     attention_mask = batch["attention_mask"].to(device)
...
#     """
#     Given per-task gradient dicts (each independently cloned), computes:
#       conflict_rate     ∈ [0, 1]
#       conflict_severity ∈ [0, 1]   (0 if no conflicts)
#       gradient_variance ∈ [0, ∞)   mean per-param variance across tasks
...
#         # Stack → (n_tasks, D); var across task dim (dim=0).
#         # unbiased=True (default) requires n_tasks >= 2, which we guaranteed above.
#         stacked = torch.stack(grads_for_param, dim=0)   # (n_tasks, D)
#         param_variance = stacked.var(dim=0, unbiased=True).mean().item()
...
#             norm2 = g2.norm()
#             if norm1 < 1e-12 or norm2 < 1e-12:
#                 continue  # skip zero-gradient params
...
#     gradient_variance is soft-normalised with tanh so all three components
#     live in [0, 1] before weighting.
#     """
